  ...
  return (
    <div className="max-w-3xl mx-auto space-y-3">
      {faqs.map((faq) => (
        <details
          key={faq.q}
          className="group rounded-2xl border border-line bg-surface transition-colors duration-150 open:border-[#AFF45D]/50 open:shadow-sm"
        >
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 sm:px-6 sm:py-5 text-sm sm:text-base font-semibold text-ink marker:hidden [&::-webkit-details-marker]:hidden">
            ...
            <span className="relative shrink-0 grid place-items-center w-6 h-6 rounded-full border border-line text-muted transition-colors group-open:border-[#AFF45D] group-open:text-accent-ink">
              <Plus
                className="w-3.5 h-3.5 transition-opacity duration-150 group-open:opacity-0"
                aria-hidden="true"
              />
              <Minus
                className="absolute w-3.5 h-3.5 opacity-0 transition-opacity duration-150 group-open:opacity-100"
                aria-hidden="true"
              />
            </span>
          </summary>
